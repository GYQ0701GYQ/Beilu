...
res_path = '/data/guanyuqi/beilu/result/pic_005_countors5/'
...
def contours_approx(mask , img , min_epsilon):
    ...
    for i in range(len(contours)):
        ...
        if max_len < len(contours[i]):
            ...
            # print('存在第二个mask，len:' , len(second_cnt))
    
    if len(second_cnt) > 0 :   # handle second mask
        # img = cv2.drawContours(img,second_cnt,-1,(255,0,0),3)
        # print('max and next cnt:' , len(cnt) , len(second_cnt) , '  ')
        img = cv2.drawContours(img,cnt,-1,(255,0,0),5)  
        # print('second cnt:' , second_cnt)
        # print(' len second cnt:' , len(second_cnt))
        ...
        # print('两组mask边缘点' , A , B , A[2, 0] , B[3,1] ,  2*(A[2,0] - B[3,0]) - (B[1,0] - B[0,0]) , 2*(B[2,0] - A[3,0]) - (B[1,0] - B[0,0]))
        ...
        if  ( A[0,0] >= (B[1,0] - 30 ) and A[2,1] >= (B[3,1]-30) and 2*(A[2,0] - B[3,0]) < (B[1,0] - B[0,0]) ) or\
         ( B[0,0] >= (A[1,0] - 30) and B[2,1] >= (A[3,1] - 30) and 2*(B[2,0] - A[3,0]) < (B[1,0] - B[0,0]) ) :
            ...
            img = cv2.drawContours(img,second_cnt,-1,(255,0,0),3)
            img = cv2.putText(img, '2 region' , (10,300 ), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), 4)
...
if __name__=='__main__':
    ...
    if len(argv_list) == 1 and argv_list[0] == 'mov' :
        ...
        argv_list.append(r'/data/guanyuqi/beilu/result/res_005_countors_5.mp4')
